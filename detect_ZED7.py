        ...
        for i, det in enumerate(pred):
            if len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    plot_one_box([xyxy[0], xyxy[1], xyxy[2], xyxy[3]], im0, label=label, color=colors[int(cls)], line_thickness=3)
                    # Get depth at object center
                    x_center = int((xyxy[0] + xyxy[2]) / 2)
                    y_center = int((xyxy[1] + xyxy[3]) / 2)
                    depth = depth_data[y_center, x_center] #pixel value
                    depth_measurement = 1 + (((255 - depth[0])/255)*150)
                    # Check if depth is already recorded
                    # if depth_measurement in unique_depth:
                        # continue  # Skip duplicate depth value
                    # # Add depth to set
                    # unique_depth.add(depth_measurement)    
                    ...
                    if cone_z in unique_depth:
                        continue  # Skip duplicate depth value
                        # Add depth to set
                    unique_depth.add(depth_measurement) 
                    csv_writer.writerow({'Cone': cone_index , 'Depth': cone_z})
